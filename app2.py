...
# ==========================================
#  関数定義：QRコード用のテキスト整形（ここが新機能！）
# ==========================================
def format_text_for_qr(text):
    """
    AIの出力からQRコード用に不要な情報を削り、
    電子カルテが読み取りやすい形式(\r\n改行)に変換する
    """
    lines = text.split('\n')
    new_lines = []
    ...
    for line in lines:
        # 1. 氏名・生年月日の行をスキップ（データ内には不要）
        if "氏名：" in line and "生)" in line:
            continue
        
        # 2. ヘッダーを【S】【O】【P】形式に変換してシンプルに
...
def generate_response_with_fallback(chat_history, patient_name, patient_dob):
    ...
    * **生年月日:** {patient_dob}
    ...
    ### 1. 日付記載の絶対ルール
    * 「1週間前」「昨日」などの相対的な日時は、必ず本日（{today_str}）から逆算した「yyyy/mm/dd」形式に書き換えること。
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "assistant":
            with st.chat_message("assistant", avatar="👩‍⚕️"):
                # SOAPまとめの場合だけQRコードを表示
                if "■ S (Subjective)" in msg["content"]:
                    col1, col2 = st.columns([3, 1])
                    with col1:
                        st.markdown(msg["content"]) # 画面上はそのまま表示
                    with col2:
                        # ★QRには整形済みのテキストを入れる
